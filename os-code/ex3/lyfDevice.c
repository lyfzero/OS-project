...
const char* deviceName = "lyf-device";
const char* className = "lyf-device-driver";
...
static int dev_open(struct inode *inode, struct file *file)
{
    printk(KERN_INFO "LYFDevice: Device File Opened...\n");
    return 0;
}

static int dev_release(struct inode *inode, struct file *file)
{
    printk(KERN_INFO "LYFDevice: Device File Closed...\n");
    return 0;
}
...
static ssize_t dev_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
    ...
    printk(KERN_INFO "LYFDevice: Read %lu bytes, off %lu...\n", cnt, *off);
...
static ssize_t dev_write(struct file *filp, const char __user *buf, size_t len, loff_t *off)
{
    ...
    *(dev_buffer+*off) = -1;
    printk(KERN_INFO "LYFDevice: Write %lu bytes, off %lu...\n", len, *off);
...
static int __init dev_driver_init(void)
{
    ...
    if((alloc_chrdev_region(&dev, 0, 1, deviceName)) <0){
            printk(KERN_INFO "LYFDevice: Can not allocate major number!\n");
            return -1;
    }
    printk(KERN_INFO "LYFDevice: Major = %d Minor = %d \n", MAJOR(dev), MINOR(dev));
    ...
    if((cdev_add(&my_cdev, dev, 1)) < 0){
        printk(KERN_INFO "LYFDevice: Can not add the device to the system!\n");
        goto r_class;
    }

    /*Creating struct class*/
    if((dev_class = class_create(THIS_MODULE, className)) == NULL){
        printk(KERN_INFO "LYFDevice: Can not create the struct class!\n");
        goto r_class;
    }

    /*Creating device*/
    if((device_create(dev_class, NULL, dev, NULL, deviceName)) == NULL){
        printk(KERN_INFO "LYFDevice: Cannot create the device!\n");
        goto r_device;
    }
    printk(KERN_INFO "LYFDevice: Device Driver Insert!\n");
    *dev_buffer = -1;
    return 0;
...
void __exit dev_driver_exit(void)
{
    ...
    cdev_del(&my_cdev);
    unregister_chrdev_region(dev, 1);
    printk(KERN_INFO "LYFDevice: Device Driver Remove!\n");
}
